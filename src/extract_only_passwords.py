...
def extract_only_passwords(file_path=None):
    print(Fore.CYAN + "Please choose the combo file to extract passwords from..." if not file_path else "")
    if not file_path:
        Tk().withdraw()
    ...
    output_path = os.path.join(dir_name, f"{base_name}-onlypasswords.txt")
    ...
    print(Fore.GREEN + f"Unique passwords extracted: {unique_count}")
